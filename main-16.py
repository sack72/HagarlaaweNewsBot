...
# Iran-war hardcoded impacts (Saki's playbook)
IRAN_WAR_IMPACTS = [
    {"asset": "Gold", "direction": "Bearish"},
    {"asset": "Oil",  "direction": "Bullish"},
...
def apply_iran_war_override(analysis: Dict[str, Any]) -> Dict[str, Any]:
    """
    Stamp the analysis with Iran war defaults: forced High importance,
    hardcoded bias, dedicated smart header. The Somali headline from
    the AI is preserved.
    """
    analysis["category"]      = "WAR_UPDATE"
    analysis["importance"]    = "High"
    analysis["smart_header"]  = "DAGAALKA IRAN"
    analysis["impacts"]       = [dict(x) for x in IRAN_WAR_IMPACTS]
...
- ORDER: Iran war / conflict bullets FIRST → then major macro / central bank → then other notable news.
...
- If any Iran war / conflict news is present, add this EXACT final bullet:
  "• Iiraan dagaal: Gold 📉 Bearish, Oil 📈 Bullish, DXY 📈 Bullish."
...
async def build_session_bullets(items: List[Dict[str, Any]]) -> List[str]:
    ...
    has_iran = any(it.get("iran") for it in recent)
    ...
    iran_hint = "\n\nNOTE: Iran war news IS present today — include the mandatory Iran bias bullet." if has_iran else ""
    ...
    try:
        async with httpx.AsyncClient(timeout=40.0) as http_client:
            ...
            return bullets[:9]  # 8 + possible Iran bias bullet
    except Exception as e:
        logging.error(f"❌ Summary AI error: {e}")
        # Fallback: raw last-10 bullets
        fallback = [f"• {it.get('som', '')}" for it in recent[-10:] if it.get('som')]
        if has_iran:
            fallback.append("• Iiraan dagaal: Gold 📉 Bearish, Oil 📈 Bullish, DXY 📈 Bullish.")
...
def format_message(analysis: Dict[str, Any], flag: str = "", impact_dot: str = "") -> str:
    ...
    if is_iran_war:
        # Dedicated Iran war banner
        lines.append("🚨 *DEGDEG — DAGAALKA IRAN* ⚔️")
        lines.append("━━━━━━━━━━━━━━━━━━━━")
    ...
    # ----- IMPACT BLOCK (High importance ONLY) -----
    if importance == "High" and impacts:
