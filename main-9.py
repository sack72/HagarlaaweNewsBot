...
async def process_news_feed(bot: Bot):
    ...
    if new_items:
        ...
        for e in new_items:
            ...
            # Require at least a currency flag OR an impact keyword.
            # The AI classification engine handles the rest — we don't
            # want to silently drop central bank headlines like
            # "ECB's Villeroy: no rate hike expected" that lack impact keywords.
            if not flag and not impact:
                if link:
                    processed_links.add(link)
                continue

            # Default flag if only impact was detected
            if not flag:
                flag = "🌍"
            # Default impact marker (empty) if only flag was detected
            if not impact:
                impact = ""
                continue
