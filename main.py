...
# from googletrans import Translator # REMOVE: No longer needed, replaced by OpenAI
import time
import os
import pytz
import requests
...
# --- Configuration ---
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "YOUR_TELEGRAM_BOT_TOKEN") # Ensure you set this as an environment variable in Render!
TELEGRAM_CHANNEL_ID = os.getenv("TELEGRAM_CHANNEL_ID", "@HagarlaaweMarkets") # Ensure you set this as an environment variable in Render!
FINANCIAL_JUICE_RSS_FEED_URL = os.getenv("FINANCIAL_JUICE_RSS_FEED_URL", "YOUR_FINANCIAL_JUICE_RSS_FEED_URL") # Ensure you set this as an environment variable in Render!

# --- OpenAI API Key Configuration ---
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY") # ADD: Load OpenAI API key from environment variables
if not OPENAI_API_KEY:
    raise ValueError("OPENAI_API_KEY environment variable not set. Please set it in Render.")
...
# --- Global Variables ---
# translator = Translator() # REMOVE: No longer needed
...
# These are less critical now with a better translation model, but can be kept for robustness.
...
async def translate_text_with_gpt(text: str, target_language: str = "Somali") -> str:
    ...
    try:
        ...
        response = await openai.chat.completions.create(
            model="gpt-3.5-turbo",  # Or "gpt-4" if you have access and prefer higher quality/cost
            messages=[
            ...
            temperature=0.3, # Lower temperature for more deterministic and literal translation
        ...
        # Consider specific error handling here, e.g., if API key is invalid or rate limited
