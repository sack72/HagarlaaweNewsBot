...
IMPORTANT — the "headline_somali" field is the TRANSLATED NEWS ONLY:
- Write it like a human trader reporting the news in Somali — natural, confident, clear.
- DO NOT write "DEGDEG" inside the headline (the system adds that automatically).
- DO NOT write market direction (no "Gold Bullish", "DXY Bearish") inside the headline.
  Direction is handled separately in the "impacts" field for macro news only.
- Just translate and report what happened, in your natural voice.

REAL EXAMPLES OF YOUR VOICE (translated news only — no DEGDEG, no direction baked in):
- "CPI-da Maraykanka oo hoos u dhacday bishii — sicirbararka wuu qabowbay, taasoo cadaadis saaraysa Fed-ka."
- "Madaxweynaha Trump ayaa cashuur cusub ku soo rogay Shiinaha — xiisad ganacsi oo cusub ayaa bilaabatay."
- "Powell oo shir jaraa'id ka hadlay — wuxuu sheegay in sicirbararku weli sarreeyo."
- "NFP-da Maraykanka oo ka sarreysay wixii la filayay — dhaqaaluhu wuu xoogganyahay."
- "Iiraan iyo Maraykan oo gaaray heshiis hordhac ah — xiisaddii ayaa hoos u dhacday."
...
def format_message(analysis: Dict[str, Any], flag: str = "", impact_dot: str = "") -> str:
    ...
    LAYOUT RULES (new):
      • REGULAR news  → raw translated Somali headline only (with flag emoji).
                        No banner, no DEGDEG, no impact block.
      • MACRO news    → DEGDEG banner + headline + currency sentiment block.
        (MACRO_DATA / CENTRAL_BANK / MONETARY_POLICY, e.g. CPI, NFP, Fed)
      • IRAN WAR      → DEGDEG banner + headline. NO directional bias.
    """
    ...
    is_macro = category in MARKET_SIGNAL_CATEGORIES

    lines = []

    # ----- TOP BANNER: only for Iran war OR macro news -----
    if is_iran_war:
        header = smart_header or "XIISADDA IRAN IYO MARAYKANKA"
        lines.append(f"🚨 *DEGDEG — {header}* ⚔️")
        lines.append("━━━━━━━━━━━━━━━━━━━━")
    elif is_macro and importance == "High" and smart_header:
    ...
    if headline_som:
        ...
        # Macro news gets a data emoji marker so it's instantly recognizable
        data_marker = "📊 " if (is_macro and not is_iran_war) else ""
        if prefix:
            lines.append(f"{data_marker}{prefix} {headline_som}")
        else:
            lines.append(f"{data_marker}{headline_som}".strip())

    # ----- CURRENCY SENTIMENT BLOCK: macro news only -----
    if is_macro and importance == "High" and impacts:
...
async def maybe_send_banner(bot: Bot, category: str, last_message: str = ""):
    """
    Send a visual banner image if the post counter threshold is reached.
    The banner image now carries the last news post as its caption (falls
    back to the category label if no message is available).
    """
    global post_counter
    post_counter += 1
    ...
    # Caption = the last news post, if we have one. Telegram photo captions
    # are capped at 1024 chars, so trim to be safe.
    caption = (last_message or "").strip()
    if caption:
        if len(caption) > 1000:
            caption = caption[:997] + "..."
    else:
    ...
    try:
        ...
        if image_path and os.path.exists(image_path):
            # Send to Telegram — banner image WITH the last news post as caption
            with open(image_path, "rb") as img:
            ...
            # Send to Facebook
            await send_to_facebook(caption, image_path=image_path)
            logging.info(f"🖼️ Banner sent with caption: {banner_text}")
    except Exception as e:
...
async def process_news_feed(bot: Bot):
    ...
    if new_items:
        ...
        for e in new_items:
            ...
            # Maybe insert a banner (with the last news post as caption)
            await maybe_send_banner(bot, analysis.get("category", "NO_MARKET_IMPACT"), last_message=msg)
    ...
    for key, data in news_buffer.items():
        ...
        if elapsed > BUFFER_TIMEOUT_SECONDS or count >= MAX_BUFFER_SIZE:
            ...
            # Maybe insert a banner (with the last news post as caption)
            await maybe_send_banner(bot, cluster_result.get("category", "NO_MARKET_IMPACT"), last_message=msg)
