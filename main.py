...
def get_bot_state():
    try:
        ...
        if doc.exists:
            data = doc.to_dict()
            # Ensure processed_links exists as a list
            if "processed_links" not in data:
                data["processed_links"] = []
            return data
        return {"last_link": None, "last_time": 0.0, "processed_links": []}
    except Exception:
        return {"last_link": None, "last_time": 0.0, "processed_links": []}


def save_bot_state(last_link, last_time, processed_links=None):
    try:
        update_data = {"last_link": last_link, "last_time": last_time}
        if processed_links is not None:
            # Keep only the most recent 200 links to avoid unbounded growth
...
def format_message(analysis: Dict[str, Any], flag: str = "", impact_dot: str = "") -> str:
    """
    Build a clean, compact message: header + headline + impact line.
    No summary bullets, no lengthy explanations.
    """
    ...
    # Build header line
    if smart_header:
        header_line = f"{flag} {impact_dot} **{smart_header}**".strip() if impact_dot else f"{flag} **{smart_header}**".strip()
    else:
        header_emoji = CATEGORY_HEADERS.get(category, "📰 GLOBAL NEWS UPDATE")
        header_line = f"{flag} {header_emoji}".strip()
...
async def process_news_feed(bot: Bot):
    ...
    for url in RSS_URLS:
        try:
            ...
            for e in feed.entries:
                ...
                # DEDUP: Skip if we've already processed this exact link
    ...
    if new_items:
        ...
        for e in new_items:
            ...
            # FILTER RULE:
            # - If a major currency flag is detected → always process (flag is set)
            # - If no flag but has impact keyword → skip (Sweden, Taiwan etc. CPI)
            # - If neither → skip
            # This ensures only 8 major currencies + important politics get through.
            if not flag:
                if link:
                    processed_links.add(link)
                continue

            # Impact dot is optional — headlines with a flag but no impact
            # keyword (e.g. "ECB Villeroy: no rate hike") still go to the AI.
            if not impact:
                impact = ""
        ...
        save_bot_state(latest_link, latest_timestamp, processed_links=list(processed_links))
...
async def initialize_on_startup(bot: Bot):
    ...
    # --- Fast-forward state: save newest item AND all current links ---
    all_links = [e.get("link") for _, e in all_items if e.get("link")]
    save_bot_state(newest_link, newest_ts, processed_links=all_links)
    logging.info(
        f"✅ State fast-forwarded. link={newest_link}, "
        f"time={time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(newest_ts))}, "
        f"seeded {len(all_links)} links into dedup history"
    )
