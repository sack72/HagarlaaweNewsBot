...
# --- PREMIUM FEATURES (requires paid ForexNewsAPI plan) ---
# Set to "true" ONLY if you're on Premium plan
# Free Trial / Basic: keep these "false" to avoid 403 errors and wasted API calls
...
async def fetch_forexnews_articles(since_date: str = None) -> List[Dict]:
    ...
    Strategy for Free Trial:
    - Try currency pair endpoint first (most likely to work on free plan)
    - Cycles through major pairs to get broad coverage
    - Falls back to general news endpoint if pair endpoint fails
    
    In TEST_MODE: logs raw JSON for debugging.
    """
    all_articles = []

    # --- METHOD 1: Fetch by major currency pairs (works on Free Trial) ---
    # Docs: GET https://forexnewsapi.com/api/v1?currencypair=EUR-USD&items=10&token=YOUR_KEY
    priority_pairs = ["EUR-USD", "GBP-USD", "USD-JPY", "USD-CAD", "AUD-USD", "NZD-USD"]
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            for pair in priority_pairs:
                params = {
                    ...
                    "currencypair": pair,
                    "items": 5,  # 5 per pair = up to 30 total, saves API calls
                }
                ...
                try:
                    ...
                    if resp.status_code == 403:
                        logging.warning(f"⚠️ ForexNewsAPI 403 for pair {pair} — trying general endpoint instead")
                        break  # Don't burn more calls, try general endpoint
                    
                    resp.raise_for_status()
                    data = resp.json()

                    # --- DEBUG: Log raw response on first pair ---
                    if TEST_MODE and pair == priority_pairs[0]:
                        logging.info(f"🔍 DEBUG — Raw API response keys: {list(data.keys())}")
                        sample_articles = data.get("data", [])
                        if not sample_articles:
                            # Try alternative response formats
                            for possible_key in ["articles", "results", "news", "items"]:
                                sample_articles = data.get(possible_key, [])
                                if sample_articles:
                                    logging.info(f"🔍 DEBUG — Found articles under key: '{possible_key}'")
                                    break
                            if not sample_articles and isinstance(data, list):
                                sample_articles = data
                                logging.info(f"🔍 DEBUG — Response is a direct list")
                        
                        if sample_articles:
                            first = sample_articles[0]
                            logging.info(f"🔍 DEBUG — First article keys: {list(first.keys())}")
                            logging.info(f"🔍 DEBUG — Sample article:")
                            logging.info(f"    title: {first.get('title', 'N/A')}")
                            logging.info(f"    date: {first.get('date', first.get('published_at', 'N/A'))}")
                            ...
                            logging.info(f"    text: {str(first.get('text', 'N/A'))[:100]}...")
                        else:
                            logging.warning(f"⚠️ DEBUG — No articles found! Response (first 500 chars): {str(data)[:500]}")

                    articles = data.get("data", [])
                    logging.info(f"📡 ForexNewsAPI [{pair}]: {len(articles)} articles")
                    all_articles.extend(articles)

                except httpx.HTTPStatusError as e:
                    if e.response.status_code == 403:
                        logging.warning(f"⚠️ ForexNewsAPI 403 for {pair}")
                        break
                    logging.error(f"❌ ForexNewsAPI HTTP Error [{pair}]: {e.response.status_code}")
                except Exception as e:
                    logging.error(f"❌ ForexNewsAPI Error [{pair}]: {e}")

                # Small delay between pair requests to be nice to their API
                await asyncio.sleep(0.5)

    except Exception as e:
        logging.error(f"❌ ForexNewsAPI Connection Error: {e}")

    # --- METHOD 2: If no articles from pairs, try general forex news ---
    # Docs: GET https://forexnewsapi.com/api/v1/category?section=general&items=10&token=YOUR_KEY
    if not all_articles:
        logging.info("📡 Trying general forex news endpoint...")
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                params = {
                    "token": FOREXNEWS_API_KEY,
                ...
                resp = await client.get(f"{FOREXNEWS_BASE_URL}/category", params=params)
                if resp.status_code == 403:
                    logging.warning("⚠️ General news endpoint also 403 — your Free Trial may have expired")
                    logging.warning("⚠️ Check: https://forexnewsapi.com/login → verify trial status & API calls remaining")
                    return []
                resp.raise_for_status()
                data = resp.json()
                all_articles = data.get("data", [])
                logging.info(f"📡 General forex news: {len(all_articles)} articles")

                if TEST_MODE and all_articles:
                    first = all_articles[0]
                    logging.info(f"🔍 DEBUG — General news first article keys: {list(first.keys())}")
                    logging.info(f"    title: {first.get('title', 'N/A')}")
        except Exception as e:
            logging.error(f"❌ General news endpoint error: {e}")

    # --- Deduplicate by news_url or title ---
    seen = set()
    unique_articles = []
    for a in all_articles:
        key = a.get("news_url", a.get("title", ""))
    ...
    # --- Filter by timestamp (skip in TEST_MODE) ---
    if since_date and not TEST_MODE:
        unique_articles = [
            a for a in unique_articles
            if a.get("date", "") > since_date
        ]
        logging.info(f"📰 {len(unique_articles)} new articles after time filter")
...
async def fetch_economic_calendar() -> List[Dict]:
    ...
    NOTE: This endpoint requires Premium plan. Returns [] on 403.
    """
    params = {
        "token": FOREXNEWS_API_KEY,
        "date": "today",
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, params=params)
            if resp.status_code == 403:
                logging.warning("⚠️ Economic Calendar: 403 Forbidden — this endpoint requires a Premium plan. Skipping.")
                return []
            resp.raise_for_status()
            data = resp.json()
...
async def fetch_sentiment_data() -> Dict:
    ...
    NOTE: This endpoint may require Premium plan. Returns {} on 403.
    """
    params = {
        "token": FOREXNEWS_API_KEY,
    }
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(url, params=params)
            if resp.status_code == 403:
                logging.warning("⚠️ Sentiment API: 403 Forbidden — this endpoint requires a Premium plan. Skipping.")
                return {}
            resp.raise_for_status()
            return resp.json()
    except Exception as e:
...
async def main():
    ...
    if TEST_MODE:
        ...
        try:
            ...
            if ENABLE_CALENDAR:
                ...
                logging.info("📅 Economic Calendar: DISABLED (set ENABLE_CALENDAR=true for Premium plan)")
        except Exception as e:
    ...
    while True:
        try:
            ...
            # --- ECONOMIC CALENDAR (less frequent, Premium plan only) ---
            loop_count += 1
            if ENABLE_CALENDAR and loop_count % calendar_interval == 0:
                await process_economic_calendar(bot)
                loop_count = 0
            elif loop_count % calendar_interval == 0:
                loop_count = 0  # Reset counter even if calendar disabled
